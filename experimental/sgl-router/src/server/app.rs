...
/// Infra endpoints logged at DEBUG instead of INFO: Prometheus scrapes
/// `/metrics` and the kubelet polls `/healthz` + `/readyz` every few seconds,
/// so INFO lines for them would bury real API traffic.
...
///
/// Also the single access-log site: exactly one `http_request` line per
/// request, carrying the final status — including responses produced before
/// any handler runs (a 413 from the body-limit layer) and handler
/// short-circuits that return via `?`. For streaming responses `duration_ms`
/// is time to response headers; stream duration lives in
