...
///
/// `transport_ok` is byte-level stream health. The other fields separate
/// outcomes that are byte-level indistinguishable from success: an engine
/// that commits `200 OK`, reports failure as an in-band `data: {"error"...}`
/// event, and closes cleanly; and a client that walked away mid-generation.
...
/// Carryover cap for the in-band scanner. A single line longer than this
/// without a newline resets the buffer — detection of that one pathological
/// line is forfeited, but memory stays bounded.
const INBAND_SCAN_CARRYOVER_CAP: usize = 1 << 20; // 1 MiB

/// Incremental scanner for in-band SSE error envelopes.
///
/// The engine commits `200 OK` at first byte; a failure after that point can
/// only be reported in-band, as `data: {"error": ...}\n\n` (the shape
/// sglang's `create_streaming_error_response` emits). Complete lines within
/// a chunk are scanned in place; only a trailing partial line is copied into
/// the carryover. The prefix check is parity-safe: inside a JSON string
/// value every quote is escaped, so the raw bytes `{"error"` cannot start a
/// well-formed data payload unless it really is an error envelope.
#[derive(Default)]
...
///
/// # Completion hook
/// When `on_complete` is `Some`, the closure runs exactly once when the pump
/// task finishes, receiving a [`StreamEnd`]: `transport_ok` is `true` on
/// clean stream end (including a clean client disconnect after the headers
/// landed), `false` on upstream stream error or pump panic;
/// `saw_inband_error` reports a well-formed `data: {"error"...}` SSE event
/// riding the stream; `client_disconnect` reports the client dropping the
/// body early. `forward_streaming_to` records the worker's circuit-breaker
/// outcome from `transport_ok` alone — an in-band error is an
/// application-level verdict, not a transport fault — without which a worker
/// that returns 2xx headers and then drops mid-flight would stay credited as
/// healthy. The in-band scan runs only when `on_complete` is installed.
///
/// # First-byte hook
...
/// non-empty chunk arrived — inter-token latency as seen at the router. Gaps
/// are measured between upstream ARRIVALS, so the reading is engine pacing,
/// not client drain speed, while the 64-slot channel has room.
/// `forward_streaming_to` passes a closure that records
/// `sgl_router_itl_seconds` for 2xx streaming responses.
    ...
    tokio::spawn(async move {
        ...
        let pump = AssertUnwindSafe(async move {
            ...
            while let Some(chunk) = s.next().await {
                ...
                match &item {
                    Ok(bytes) => {
                        // Fire the time-to-first-token hook on the first
                        // successful chunk. `take()` makes it fire at most
                        // once; an error-first stream never produced a token,
                        // so it's left unfired (and dropped on task end).
                        if let Some(hook) = on_first_byte.take() {
                            hook();
                        }
                        // Inter-token latency: gap between successive
                        // non-empty Ok-chunk arrivals. The first chunk seeds
                        // the clock (its latency is TTFT, not ITL).
                        if !bytes.is_empty() {
...
mod tests {
    ...
    fn inband_scanner_ignores_error_text_inside_content() {
        ...
        // sequence cannot start a content payload.
        let mut s = InbandErrorScanner::default();
        s.feed(
